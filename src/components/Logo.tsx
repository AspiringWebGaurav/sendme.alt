...
export function Logo({ size = 'sm', withLink = false }: LogoProps) {
 ...
 <div className={`${s.container} bg-bg-surface border border-border-subtle flex items-center justify-center font-bold ${s.font} text-text-primary`}>
 S
 </div>
 <span className={`font-semibold ${s.text} tracking-tight text-text-primary`}>
 sendme.alt
 </span>
 </div>
 )
