...
export function SuccessView({ mode, fileName, fileSize, onReset, receivedBlob }: SuccessViewProps) {
 ...
 className="text-2xl sm:text-3xl font-semibold text-text-primary mb-3 text-center relative z-10"
 >
 ...
 className="text-sm sm:text-base text-text-secondary text-center mb-8 max-w-[280px] break-all relative z-10"
 >
 {fileName} {fileSize ? <span className="text-text-muted whitespace-nowrap ml-1">({(fileSize / 1024 / 1024).toFixed(2)} MB)</span> : ''}
 </motion.p>
 ...
 onClick={onReset}
 className="px-8 py-3.5 bg-bg-surface hover:bg-bg-elevated text-text-primary rounded-full font-medium transition-colors shadow-sm relative z-10"
 >
 ...
 onClick={handleSave}
 className="px-8 py-3.5 bg-emerald-600 hover:bg-emerald-500 text-text-primary rounded-full font-medium transition-colors shadow-sm relative z-10 mb-3"
 >
 Save File
 </motion.button>
 )}
